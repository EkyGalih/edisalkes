    ...
    if request.POST:
        if "invoice_no" in request.POST:
            ...
            if frm_ven.is_valid():
                frm_ven.save()
                messages.add_message(request, messages.INFO,
                                     mark_safe('vendor added.'))
                status_ven = 1
            print(frm_ven.errors)

    context = {"frm": frm, "purcs": purchase_list,
               'frm_ven': frm_ven, 'status_ven': status_ven}
...
def edittemplateitemps(request, pk):
    ...
    # print(frm)
    html = render_to_string("backend/particial-modal/form-edit-template-ps.html", {
                            "frm": frm, "trs": tr_obj, "stts": mstat, 'stok': stok})
    data = {'html': html}
...
def edititempurchase(request, pk):
    ...
    if request.POST:
        ...
        if frm2.is_valid():
            ...
            tr2_obj.total_amount = float(
                tr2_obj.total) + ongkos_kirim + tr2_obj.tax
...
def deleteItemPurchase(request, pk):
    ...
    if request.POST:
        try:
            ...
            purchase.total_amount = float(
                purchase.total) + ongkos_kirim + float(purchase.tax)
...
def sales(request):
    ...
    if "q" in request.GET and request.GET["q"] != "":
        sale_list = Sales.objects.filter(
            created_by=request.user, invoice_no=request.GET['q'])
    if request.POST:
        print("berhasil masuk post")
        if "transaction_date" in request.POST:
            ...
            if frm_pel.is_valid():
                frm_pel.save()
                messages.add_message(request, messages.INFO,
                                     mark_safe('pelanggan added.'))
                status_pel = 1
    context = {"frm": frm, "sls": sale_list, "frm_pel": frm_pel,
               "status_pel": status_pel, "pic": pic}
...
def salesDetail(request, pk):
    ...
    context = {"trs": tr_obj, "frm2": frm2, "sale_item": sale_item, "frm": frm,
               "kar": kar, "pel": pel, "due_date": due_date, 'persen_pajak': persen_pajak}
...
def edittemplateitem(request, pk):
    ...
    stok = Stok.objects.all()
    # print(frm)
    html = render_to_string("backend/particial-modal/form-edit-item.html",
                            {"frm": frm, "trs": tr_obj, "stts": mstat, 'stok': stok})
    data = {'html': html}
    return JsonResponse(data)
