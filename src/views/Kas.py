...
def edittemplatekkMasuk(request, pk):
    ...
    # print(frm)
    html = render_to_string("backend/particial-modal/form-edit-kaskecil-masuk.html", {
                            "frm": kkm, "trs": kkm_obj, "totalKas": t_list})
    data = {'html': html}
...
def kaskecilDetailKeluar(request, pk):
    ...
    beban = Beban.objects.all()
    pkt = "kas123"
    t_list = TotalKasBesarKeluar.objects.filter(id=pkt)
    ...
    if request.POST:
        ...
        frm3 = TotalKasBesarKeluarForm(request.POST)
    ...
    context = {"trs": tr_obj, "frm2": frm2,
               "kaskecil_item": kaskecil_item, "frm": frm, "kar": kar, "beban": beban, "totalKas": t_list}
